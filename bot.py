"""
Username Info Bot — v17 FINAL
+ Username → UserID → API (multi-layer resolver with cache)
+ Smooth animations everywhere (hides all delays)
+ Advanced admin panel
+ Telethon session integrated
"""
...
def _ensure_modules():
    ...
    miss = [(m, p) for m, p in _PACKAGES.items()
...
logger = logging.getLogger("uinfo_v17")
...
BACKUP_CHAT_ID = _clean_env("BACKUP_CHAT_ID")
AUTO_BACKUP_HOURS = int(_clean_env("AUTO_BACKUP_HOURS", "0") or "0")
...
try:
    ...
    DAILY_FREE_CREDITS = int(_clean_env("DAILY_FREE_CREDITS", "10"))
...
cache_stats    = data.setdefault("cache_stats",
    {"hits": 0, "misses": 0, "bot_chat": 0, "telethon": 0, "numeric": 0})
...
def _init_telethon():
    ...
    if not (TG_API_ID and TG_API_HASH and TG_SESSION_STR):
        logger.warning("📱 Telethon: creds missing — username→ID disabled")
        return False
    try:
    ...
    try:
        ...
        _tg_loop = loop; _tg_client = client
        _tg_me = me; _tg_ready = True
        return True
    except Exception as e:
    ...
    async def _resolve():
        try:
            entity = await asyncio.wait_for(
                _tg_client.get_entity(f"@{uname}"), timeout=10)
...
# ══════════════════════════════════════════════════════════════
#  ANIMATION ENGINE — v17 (smooth + everywhere)
# ══════════════════════════════════════════════════════════════
SPINNER = ["⠋","⠙","⠹","⠸","⠼","⠴","⠦","⠧","⠇","⠏"]
SPINNER2 = ["◐","◓","◑","◒"]
SPINNER3 = ["⣾","⣽","⣻","⢿","⡿","⣟","⣯","⣷"]
    ...
    def _run():
        try:
            ...
            while time.time() < end:
                ...
                time.sleep(min(4, end - time.time()) or 0.5)
        except: pass
...
class AnimMsg:
    """Smooth edit-based animation. Auto-recovers if message edited elsewhere."""
...
def frames_resolve(username=""):
    """Smooth resolution animation — hides 3-10s Telethon delay."""
    out = []
    u = esc(username)
    # Phase 1: Contacting
    for s in SPINNER[:6]:
    ...
    for s in SPINNER2:
        out.append(f"<b>{s} Scanning multiple sources…</b>\n"
                   f"<i>{u}</i>\n<code>{progress_bar(58)}</code>")
    # Phase 4: Converting
    for i, pct in enumerate(range(65, 92, 5)):
        out.append(f"<b>🔄 Converting to user ID</b>{DOTS[i%5]}\n"
                   f"<i>{u}</i>\n<code>{progress_bar(pct)}</code>")
    # Phase 5: Finalizing
    for s in SPINNER3[:4]:
...
def frames_verify_resolved(uname="", uid=""):
    """Brief success animation after resolution."""
    return [
        f"✅ <b>Username found</b>",
        f"✅ <b>Username found</b> ✓",
        f"✅ <b>Username found</b> ✓✓",
        f"🆔 <b>Converting to ID</b>",
        f"🆔 <b>Converting to ID</b> .",
        f"🆔 <b>Converting to ID</b> ..",
        f"🎯 <b>ID:</b> <code>{esc(uid)}</code>",
    ]
...
def resolve_to_user_id(query):
    ...
    Returns (user_id_str, display, source, error_msg).
    """
    ...
    if query.isdigit():
        ...
        return None, query, None, (
            ...
            f"<code>{esc(query)}</code> valid Telegram user ID nahi hai.\n"
    ...
    try:
        ...
        emsg = str(e).lower()
        if "not found" in emsg or "username not occupied" in emsg:
            logger.info(f"⚠️ bot.get_chat: @{uname} not found → Telethon")
        else:
            logger.info(f"⚠️ bot.get_chat failed → Telethon")
    except Exception as e:
        logger.info(f"⚠️ bot.get_chat exception → Telethon")
        ...
        try:
            ...
            if uid and uid > 0:
                ...
                return None, f"@{uname}", None, (
                    ...
                    f"<b>Reason:</b> <code>{esc(err or 'unknown')}</code>\n\n"
                    "💡 Numeric ID bhejo: <code>7030426992</code>")
        except Exception as e:
            logger.error(f"Telethon error: {e}")
            return None, f"@{uname}", None, f"Telethon error: {esc(e)}"

    return None, f"@{uname}", None, (
        "❌ <b>Resolve nahi ho paya</b>\n\n"
        "📱 <b>Telethon:</b> 🔴 OFF\n"
        "🤖 <b>bot.get_chat:</b> ❌ Failed\n\n"
        "💡 Numeric ID bhejo: <code>7030426992</code>")

# ══════════════════════════════════════════════════════════════
#  LOOKUP CORE — v17 with smooth animations
# ══════════════════════════════════════════════════════════════
...
def process_lookup(uid, cid, query, reply_to=None, is_group=False):
    """
    v17 flow with SMOOTH ANIMATIONS hiding every delay:
    1. Resolve username → user_id (animated)
    2. Show resolved card (animated)
    3. API fetch (animated)
    4. Result
    """
    try:
        query = (query or "").strip()
        ...
        if not is_numeric_input:
            ...
            # Wait minimum 2.2s so animation looks smooth even if fast
            start = time.time()
            MIN_ANIM_TIME = 2.2
            while not result_holder["done"] and (time.time() - start) < 25:
                time.sleep(0.1)
            # Ensure minimum animation runtime
            elapsed = time.time() - start
            if elapsed < MIN_ANIM_TIME:
                time.sleep(MIN_ANIM_TIME - elapsed)
        ...
        # Safety: verify positive ID
        if not _is_user_id(tg_id):
            error_shake(cid,
                f"❌ Resolved ID <code>{esc(tg_id)}</code> valid nahi hai.",
                reply_to=reply_to)
            return

        # ═══════════════════════════════════════════════════
        #  PHASE 2: API CALL with animation
        # ═══════════════════════════════════════════════════
        typing(cid, 2)
        ...
        # Run API in thread so animation is smooth during network delay
        ...
        api_thread = threading.Thread(target=_api_work, daemon=True)
        api_thread.start()
        ...
        while not api_holder["done"] and (time.time() - start) < 20:
            time.sleep(0.1)
        elapsed = time.time() - start
        if elapsed < MIN_API_ANIM:
            time.sleep(MIN_API_ANIM - elapsed)
...
def cb_bc_yes(c):
    try:
        ...
        if not payload:
            bot.answer_callback_query(c.id, "❌ Expired. Resend.")
            return
...
def cmd_help(m):
    try:
        ...
        bot.reply_to(m, "\n".join([
            ...
            "/tgstatus — Telethon status (admin)",
...
def cmd_tgstatus(m):
    try:
        ...
        lines = ["📱 <b>Telethon Status</b>", "<code>━━━━━━━━━━━━━━━━━━━━</code>",
                 f"<b>Ready:</b> {'🟢 YES' if _tg_ready else '🔴 NO'}",
                 f"<b>API ID:</b> <code>{TG_API_ID or 'missing'}</code>",
                 f"<b>Session:</b> {'✅ set' if TG_SESSION_STR else '❌ missing'}",
                 f"<b>Mode:</b> <code>{USERNAME_RESOLVER}</code>"]
        if _tg_me:
            lines += ["", "✅ <b>Logged in:</b>",
                      f"  👤 @{esc(_tg_me.username or 'no_username')}",
                      f"  🆔 <code>{_tg_me.id}</code>"]
        lines += ["", f"💾 <b>Cache entries:</b> {len(username_cache)}"]
        bot.reply_to(m, "\n".join(lines))
...
def cmd_cache(m):
    try:
        ...
        num = cache_stats.get("numeric", 0)
        tl = hits + misses
        hr = (hits / tl * 100) if tl else 0
        lines = ["💾 <b>CACHE STATS</b>", "<code>━━━━━━━━━━━━━━━━━━━━</code>",
                 f"📦 Cached: <b>{total}</b>",
                 f"🎯 Hits: <b>{hits}</b>",
                 f"❌ Misses: <b>{misses}</b>",
...
def cmd_anim(m):
    try:
        ...
        flash(m.chat.id, "🎬 <b>Animation engine v17 online</b>", reply_to=m.message_id)
...
def btn_info(m):
    try:
        ...
        tg = "🟢 Telethon ON (any username)" if _tg_ready else "🔴 Telethon OFF"
        flash(m.chat.id,
            f"📌 Send <b>username</b> or <b>Numeric ID</b>\n\n"
            f"📱 <b>Resolver:</b> {tg}", reply_to=m.message_id)
...
def btn_admin(m):
    try:
        typing(m.chat.id, 0.5)
        flash(m.chat.id, "👑 <b>Admin Panel v17</b>", reply_to=m.message_id)
        bot.send_message(m.chat.id, "Choose action:", reply_markup=admin_kb())
...
def btn_dash(m):
    try:
        ...
        lines = ["📊 <b>D A S H B O A R D</b>",
                 ...
                 f"📱 Telethon: <b>{tg}</b>",
                 f"💾 Cache entries: <b>{len(username_cache)}</b>",
                 ...
                 f"🌐 API: <b>{'🟢 ON' if settings.get('api_enabled', True) else '🔴 OFF'}</b>", "",
                 f"👑 <b>Admins ({len(get_all_admins())}):</b>",
                 admins_list]
...
def btn_bcast(m):
    try:
        msg = bot.reply_to(m, "Broadcast text bhejo (max 4000):")
...
def btn_dm(m):
    try:
        msg = bot.reply_to(m, "User ID bhejo:")
...
def _dm3(m, tgt):
    try:
        ...
        try:
            ...
            log_action("dm_user", m.from_user.id, f"to={tgt}")
        except Exception as e:
...
def btn_srch(m):
    try:
        msg = bot.reply_to(m, "Username/name/ID bhejo:")
...
def _srch2(m):
    try:
        ...
        for k, u in results[:15]:
            ...
            un = (u.get("username") or "no_user")[:15]
            lines.append(f"• <code>{k}</code> @{esc(un)} ({esc(nm)}) — 💎{u.get('credits',0)}")
...
def btn_topref(m):
    try:
        ...
        lines = ["🏆 <b>TOP 10 REFERRERS</b>", "<code>━━━━━━━━━━━━━━━━━━━━</code>"]
        medals = ["🥇","🥈","🥉","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣","9️⃣","🔟"]
        for i, (k, u) in enumerate(top):
            nm = (u.get("first_name") or "?")[:15]
...
def btn_backup(m):
    try:
        ...
        bot.send_document(m.chat.id, buf,
                          ...
                          parse_mode="HTML")
        log_action("backup", m.from_user.id)
    except Exception as e:
...
def btn_restore(m):
    try:
        msg = bot.reply_to(m, "⚠️ Backup .json bhejo (data replace hoga):")
...
def _restore2(m):
    try:
        ...
        log_action("restore", m.from_user.id)
        bot.send_message(m.chat.id, "Admin Panel:", reply_markup=admin_kb())
    except Exception as e:
...
def btn_fjmgr(m):
    try:
        ...
        bot.send_message(m.chat.id, "Choose action:", reply_markup=fj_kb())
...
def _addfj2(m):
    try:
        ...
        log_action("add_fj", m.from_user.id, cid)
...
def _remfj2(m):
    try:
        ...
        if removed:
            _validate_fj()
            flash(m.chat.id, f"✅ Removed <code>{cid}</code>", reply_to=m.message_id)
            log_action("rem_fj", m.from_user.id, cid)
        else:
...
def btn_admmgr(m):
    try:
        ...
        bot.send_message(m.chat.id, "Choose action:", reply_markup=admin_mgr_kb())
...
def _addadm2(m):
    try:
        ...
        log_action("add_admin", m.from_user.id, str(uid))
        try: bot.send_message(uid, "🎉 Aapko admin banaya gaya! /start karein.")
        except: pass
    ...
    try:
        ...
        with _data_lock:
            if uid in dynamic_admins:
                ...
                log_action("rem_admin", m.from_user.id, str(uid))
            else:
...
def btn_admlist(m):
    try:
        lines = ["👑 <b>ALL ADMINS</b>", "<code>━━━━━━━━━━━━━━━━━━━━</code>",
                 f"Env: {len(ADMIN_IDS)} | Dynamic: {len(dynamic_admins)}", ""]
        for i, a in enumerate(get_all_admins(), 1):
...
def _a_add2(m):
    try:
        ...
        tgt = int(m.text.strip())
        msg = bot.reply_to(m, f"Amount to ADD for <code>{tgt}</code>:", parse_mode="HTML")
...
def _a_add3(m, tgt):
    try:
        ...
        log_action("add_credits", m.from_user.id, f"to={tgt} amt={amt}")
        bot.send_message(m.chat.id, "Credit Manager:", reply_markup=credit_kb())
    except ValueError:
        error_shake(m.chat.id, "Bad amount.", reply_to=m.message_id)
...
def _a_rem2(m):
    try:
        ...
        if not m.text.strip().isdigit(): error_shake(m.chat.id, "Bad.", reply_to=m.message_id); return
        tgt = int(m.text.strip())
        msg = bot.reply_to(m, f"Amount to REMOVE from <code>{tgt}</code>:", parse_mode="HTML")
...
def _a_rem3(m, tgt):
    try:
        ...
        animate_credit(m.chat.id, old, new, reply_to=m.message_id)
        flash(m.chat.id, f"✅ -{amt} → <code>{tgt}</code> = <b>{new}</b>", reply_to=m.message_id)
        log_action("rem_credits", m.from_user.id, f"to={tgt} amt={amt}")
        bot.send_message(m.chat.id, "Credit Manager:", reply_markup=credit_kb())
    except ValueError:
        error_shake(m.chat.id, "Bad.", reply_to=m.message_id)
...
def _a_set3(m, tgt):
    try:
        ...
        log_action("set_credits", m.from_user.id, f"to={tgt} val={v}")
        bot.send_message(m.chat.id, "Credit Manager:", reply_markup=credit_kb())
    except ValueError:
        error_shake(m.chat.id, "Bad.", reply_to=m.message_id)

@bot.message_handler(func=lambda m: _private_only(m) and _has_user(m)
                     and is_admin(m.from_user.id) and _btn_match(m, "💎 Bulk Add"))
def btn_bulkadd(m):
    try:
        msg = bot.reply_to(m,
            "Format: <code>amount</code> (sabko) ya <code>amount,id1,id2</code>",
            parse_mode="HTML")
        bot.register_next_step_handler(msg, _bulk2)
    except Exception as e: logger.error(f"btn_bulkadd: {e}")
...
def _bulk2(m):
    try:
        ...
        amt = int(parts[0])
        if amt <= 0: error_shake(m.chat.id, "> 0", reply_to=m.message_id); return
        targets = parts[1:] if len(parts) > 1 else list(users.keys())
        total = len(targets); ok = 0
        pm = bot.send_message(m.chat.id, f"💎 Processing {total}…")
        for i, t in enumerate(targets, 1):
            try: add_credits(t, amt); ok += 1
            except: pass
            if i % 20 == 0:
                try: bot.edit_message_text(f"💎 {i}/{total} (ok={ok})",
                                            m.chat.id, pm.message_id)
                except: pass
        try: bot.edit_message_text(f"✅ Done! {ok}/{total} × +{amt}",
                                    m.chat.id, pm.message_id)
        except: pass
        log_action("bulk_add", m.from_user.id, f"amt={amt} ok={ok}/{total}")
        bot.send_message(m.chat.id, "Credit Manager:", reply_markup=credit_kb())
    except Exception as e:
        logger.error(f"_bulk2: {e}")
        error_shake(m.chat.id, f"Bad: {esc(e)}", reply_to=m.message_id)

@bot.message_handler(func=lambda m: _private_only(m) and _has_user(m)
                     and is_admin(m.from_user.id) and _btn_match(m, "🔄 Reset User"))
def btn_resetusr(m):
    try:
        msg = bot.reply_to(m, "User ID reset karne ke liye:")
        bot.register_next_step_handler(msg, _resetusr2)
    except Exception as e: logger.error(f"btn_resetusr: {e}")
...
def _resetusr2(m):
    try:
        ...
        if _is_any_button(m.text): bot.reply_to(m, "❌", reply_markup=credit_kb()); return
        if not m.text.strip().isdigit():
            error_shake(m.chat.id, "Bad.", reply_to=m.message_id); return
        k = m.text.strip()
        if k not in users:
            error_shake(m.chat.id, "Not found.", reply_to=m.message_id); return
        with _data_lock:
            users[k] = {"credits": SIGNUP_BONUS, "searches": 0, "referrals": 0,
                        "referred_by": None, "banned": False, "last_daily_claim": "",
                        "joined_at": datetime.now().isoformat(),
                        "last_seen": datetime.now().isoformat(),
                        "first_name": "", "username": "", "total_searches": 0,
                        "spent_credits": 0, "earned_credits": SIGNUP_BONUS}
            save_data()
        flash(m.chat.id, f"✅ Reset <code>{k}</code>", reply_to=m.message_id)
        log_action("reset_user", m.from_user.id, k)
        bot.send_message(m.chat.id, "Credit Manager:", reply_markup=credit_kb())
    except Exception as e: logger.error(f"_resetusr2: {e}")
...
def _bn2(m):
    try:
        ...
        flash(m.chat.id, f"🚫 Banned <code>{m.text.strip()}</code>", reply_to=m.message_id)
        log_action("ban", m.from_user.id, m.text.strip())
...
def _ubn2(m):
    try:
        ...
        flash(m.chat.id, f"✅ Unbanned <code>{m.text.strip()}</code>", reply_to=m.message_id)
        log_action("unban", m.from_user.id, m.text.strip())
        bot.send_message(m.chat.id, "User Manager:", reply_markup=user_kb())
...
def btn_exp(m):
    try:
        ...
        w.writerow(["uid","first_name","username","credits","searches","referrals",
                    "referred_by","banned","last_daily_claim","joined_at","last_seen",
                    "total_searches","spent_credits","earned_credits"])
        for k, u in users.items():
            w.writerow([k, u.get("first_name",""), u.get("username",""),
                        ...
                        u.get("last_daily_claim",""),
                        u.get("joined_at",""), u.get("last_seen",""),
                        u.get("total_searches",0), u.get("spent_credits",0),
                        u.get("earned_credits",0)])
        ...
        bot.send_document(m.chat.id, bio,
                          caption=f"📤 <b>{len(users)}</b> users",
                          parse_mode="HTML")
        log_action("export_csv", m.from_user.id)
    except Exception as e:
...
def btn_udetails(m):
    try:
        ...
        lines = ["📊 <b>AGGREGATE</b>", "<code>━━━━━━━━━━━━━━━━━━━━</code>",
                 ...
                 f"💎 Credits in system: <b>{tc}</b>",
                 f"💰 Total Earned: <b>{te}</b>",
                 f"💸 Total Spent: <b>{ts_}</b>",
                 f"👥 Total Referrals: <b>{tr}</b>",
                 f"🚫 Banned: <b>{len(banned)}</b>",
                 f"🔥 Active 24h: <b>{active_users_24h()}</b>"]
        reveal_lines(m.chat.id, lines, interval=0.10, reply_to=m.message_id,
                     final_markup=user_kb())
...
def btn_m_on(m):
    try:
        ...
        flash(m.chat.id, "🔧 Maintenance: <b>ON</b>", reply_to=m.message_id)
        log_action("maint_on", m.from_user.id)
...
def btn_m_off(m):
    try:
        ...
        flash(m.chat.id, "✅ Maintenance: <b>OFF</b>", reply_to=m.message_id)
        log_action("maint_off", m.from_user.id)
        bot.send_message(m.chat.id, "Settings:", reply_markup=settings_kb())
...
def btn_apitgl(m):
    try:
        ...
        flash(m.chat.id, f"🌐 API: <b>{'🔴 OFF' if cur else '🟢 ON'}</b>",
              reply_to=m.message_id)
        log_action("api_toggle", m.from_user.id, f"new={not cur}")
        bot.send_message(m.chat.id, "Settings:", reply_markup=settings_kb())
...
def btn_clrcache(m):
    try:
        with _data_lock:
            c = len(username_cache); username_cache.clear(); save_data()
        flash(m.chat.id, f"🧹 Cleared <b>{c}</b>", reply_to=m.message_id)
        log_action("clear_cache", m.from_user.id, f"count={c}")
        bot.send_message(m.chat.id, "Settings:", reply_markup=settings_kb())
...
def _boot_sequence():
    boot = ["  ╔══════════════════════════════════════╗",
            "  ║  🤖 UINFO BOT  v17 — FINAL           ║",
            "  ║  Smooth animations + full resolver   ║",
            "  ╚══════════════════════════════════════╝"]
...
if __name__ == "__main__":
    ...
    logger.info("=" * 60)
    logger.info("🚀 v17 FINAL — Smooth animations everywhere")
    ...
    if _init_telethon():
        ...
        logger.warning("⚠️ Telethon OFF — username resolution limited")
